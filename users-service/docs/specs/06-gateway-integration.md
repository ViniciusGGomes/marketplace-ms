...
- O usuário autenticado está disponível em `@CurrentUser()` com `{ id, email, role }` (extraído do token JWT)
...
- `AuthController` com `POST /auth/register` e `POST /auth/login`  que delegam para o `AuthService` — **PROBLEMA: contorna o ProxyService**
...
- `HealthCheckService` que chama `GET /health` em cada microsserviço para verificar disponibilidade
...
- Utiliza os dados do usuário já extraídos pelo guard (`@CurrentUser()`) para compor a resposta
- Retorna um objeto com: `userId`, `email` e `role`
- Finalidade: permitir que o api-gateway (ou outros microsserviços) valide tokens e obtenha informações do usuário autenticado sem precisar decodificar o JWT localmente
...
- Finalidade: ser consumido pelo `HealthCheckService` do api-gateway, que periodicamente chama `GET /health` nos microsserviços
...
Criar um `UsersController` no gateway que utilize `ProxyService.proxyRequest('users', ...)` para **todas** as rotas de autenticação e usuários.
...
- Autenticação entre microsserviços (service-to-service auth)
