...
- Possui o decorator customizado `@CurrentUser()` em `src/auth/decorators/current-user.decorator.ts` para extração limpa e tipada de claims (ex: `@CurrentUser('id')`) sem expor o objeto `req` do Express.
...
- Utiliza o decorator `@CurrentUser('id')` para capturar de forma estritamente tipada o `id` (sub do JWT) e buscar os dados **atualizados** do usuário no banco de dados
...
└── src/
    ├── auth/
    │   └── decorators/
    │       └── current-user.decorator.ts # (novo/alterado) decorator para tipagem do req.user
    └── users/
...
- Cache de consultas
- Integração síncrona com outros microsserviços
- Seeds ou migrations
- CI/CD
