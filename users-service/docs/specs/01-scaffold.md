...
Configurar o scaffold do microsserviço `users-service` dentro do projeto `marketplace-ms`, incluindo conexão com banco de dados PostgreSQL, definição da entidade `User`, Docker Compose para o banco e configuração base do NestJS seguindo o padrão dos demais serviços (`checkout-service`, `payments-service`).
