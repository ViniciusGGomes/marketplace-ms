...
Configurar o scaffold do microsserviço `products-service` dentro do projeto `marketplace-ms`, incluindo conexão com banco de dados PostgreSQL, definição da entidade `Product`, Docker Compose para o banco e configuração base do NestJS seguindo o padrão dos demais serviços (especialmente `users-service`).
